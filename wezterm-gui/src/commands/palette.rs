use super::{CommandDef, ExpandedCommand, derive_command_from_key_assignment};
...
pub fn actions_for_palette_and_menubar(
    ...
    let mut result = CommandDef::expanded_commands(config);
...
pub fn recreate_menubar(config: &ConfigHandle, inputmap: &InputMap) {
    ...
    let mut commands = actions_for_palette_and_menubar(config, inputmap);
